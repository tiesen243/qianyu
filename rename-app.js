...
if (!/^[a-z][a-z0-9-]*$/.test(newName)) {
  console.error("Error: newName must be lowercase alphanumeric and hyphens, starting with a letter.")
  process.exit(1)
}
...
function findFiles(dir, filename) {
  ...
  for (const entry of readdirSync(dir)) {
    if (entry === "node_modules" || entry === ".git" || entry === ".bun" || entry.startsWith(".conform")) continue
...
// ---------------------------------------------------------------------------
replaceInFile(join(root, "apps/desktop/src-tauri/tauri.conf.json"), (content) => {
  const json = JSON.parse(content)
  if (json.productName === oldTitle || json.productName === oldName) json.productName = newTitle
  if (json.identifier === `com.${oldName}.desktop`) json.identifier = `com.${newName}.desktop`
  if (json.app?.windows) {
    for (const win of json.app.windows) {
      if (win.title === oldTitle || win.title === oldName) win.title = newTitle
    }
  }
  return JSON.stringify(json, null, 2) + "\n"
})
...
replaceInFile(join(root, "apps/mobile/android/settings.gradle"), (content) =>
  applyReplacements(content, [[`rootProject.name = '${oldName}'`, `rootProject.name = '${newName}'`]]),
)
...
if (existsSync(oldAndroidPkgDir)) {
  ...
  if (!existsSync(newParent)) {
    ...
    console.log(`  renamed: ${oldParent.replace(root + "/", "")} -> ${newParent.replace(root + "/", "")}`)
  }
}
...
if (existsSync(iosOldDir)) {
  ...
  console.log(`  renamed: ${iosOldDir.replace(root + "/", "")} -> ${iosNewDir.replace(root + "/", "")}`)
}
...
replaceInFile(pbxprojPath, (content) =>
  applyReplacements(content, [
    [`com.${oldName}.mobile`, `com.${newName}.mobile`],
    [`INFOPLIST_KEY_CFBundleDisplayName = "${oldTitle}"`, `INFOPLIST_KEY_CFBundleDisplayName = "${newTitle}"`],
    [`INFOPLIST_KEY_CFBundleDisplayName = "${oldName}"`, `INFOPLIST_KEY_CFBundleDisplayName = "${newName}"`],
    ...
    [`/* Build configuration list for PBXNativeTarget "${oldName}" */`, `/* Build configuration list for PBXNativeTarget "${newName}" */`],
    [`/* Build configuration list for PBXProject "${oldName}" */`, `/* Build configuration list for PBXProject "${newName}" */`],
    [`name = "${oldName}"`, `name = "${newName}"`],
...
replaceInFile(join(root, "apps/mobile/ios/Podfile"), (content) =>
  applyReplacements(content, [[`target '${oldName}' do`, `target '${newName}' do`]]),
)
